...
function Service(props){
  ...
  return (
    <div className={`flex flex-col items-center font-Montserrat border-[3px] p-7 w-80 h-96 gap-5 ${borderStyle}`}>
...
function Services() {
  return (
    <section className='dark-blue-gradient flex flex-col justify-center items-center gap-14 p-36'>
      <div className='text-beige font-Montserrat  text-7xl'><span className='text-light-blue'>Our </span>services</div>
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5'>{items.map((item) =><Service icon={item.icon}
               title={item.title}
               description={item.description}
               borderStyle={item.borderStyle}/>)}
      </div>
      <Button bc='border-black' tc='text-white' bgc='bg-blue-400'>Learn more</Button>
    </section>
  )
}
